...
//  iDraft
//
//  Created by yujin on 2020/11/07.
...
class FileUploadViewController: UIViewController,UIDocumentPickerDelegate,UINavigationControllerDelegate,FileManagerDelegate{
    ...
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentAt url: URL) {
        ...
        if editJson["body"] != nil{
            ...
            editJson["cover_audio"] = audioFileName
            editJson["cover_image"] = imageFileName
        }else{
            vcP.iCloudBody = ""
        }
        vcP.activeTab = 2
    ...
    func createnewSharedlink(){
        let client = DropboxClientsManager.authorizedClient
        client?.sharing.createSharedLinkWithSettings(path: "/test.txt")
            .response { response, error in
                if let response = response {
                    print(response)
                } else if let error = error {
        ...
        client?.sharing.listSharedLinks(path:"/test.txt")
            .response { response, error in
                if let response = response {
                    print(response)
                } else if let error = error {
